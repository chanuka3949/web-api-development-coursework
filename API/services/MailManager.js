...
function sendDetails(order, user) {
  createDocument(order, user);
}
const sendMail = async (recepient, subject, messageBody, attachmentName) => {
  try {
...
function generateTable(doc, order) {
  ...
  for (i = 0; i < items.length; i++) {
    ...
    generateLineBreak(doc, position + 20);
    bottom = position + 40;
  }
  doc.text(`Total Amount(EUR): ${parseFloat(order.total).toFixed(2)}`, 0, bottom, {
    align: "right",
  });
}

function generateTableRow(doc, y, name, qty, unitPrice) {
  doc
    .fontSize(10)
    .text(name, 50, y)
...
function generateLineBreak(doc, y) {
  doc
    .strokeColor("#aaaaaa")
    .lineWidth(0.5)
    .moveTo(50, y)
...
function createDocument(order, user) {
